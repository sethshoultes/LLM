...
class ContextManager:
    ...
    def select_documents(self,
                      ...
                      max_documents: int = DEFAULT_MAX_DOCUMENTS) -> List[Dict[str, Any]]:
        """
        ...
        for doc in documents:
            ...
            if doc_tokens <= remaining_tokens:
                ...
                if not selected_docs or remaining_tokens > doc.get("header_tokens", 0) + 50:
                    ...
                    if content_tokens_available > 50:  # Only truncate if we can include meaningful content
                        truncated_content = self._truncate_text(
                            doc.get("content", ""),
                            content_tokens_available
                        )
    ...
    def format_context(self,
                     documents: List[Dict[str, Any]],
                     query: str = "",
                     add_prefix: bool = True) -> Tuple[str, List[Dict[str, Any]]]:
        """
    ...
    @timer
    def prepare_context_for_prompt(self,
                                documents: List[Dict[str, Any]],
                                query: str,
                                system_message: str,
                                messages: List[Dict[str, Any]],
                                max_documents: int = DEFAULT_MAX_DOCUMENTS) -> Tuple[str, List[Dict[str, Any]]]:
        """
