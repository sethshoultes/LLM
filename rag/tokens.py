...
class TokenManager:
    ...
    def estimate_prompt_tokens(
        self, messages: List[Dict[str, Any]], system_message: str = "", model_id: str = None
    ) -> Dict[str, int]:
        """
    ...
    def allocate_tokens(
        ...
        available_for_context = max(
            0, context_window - system_tokens - message_tokens - reserved_tokens
        )
        ...
        reserved_pct = (
            round((reserved_tokens / context_window) * 100, 1) if context_window > 0 else 0
        )
        available_pct = (
            round((available_for_context / context_window) * 100, 1) if context_window > 0 else 0
        )
        ...
        return {
            ...
            "total_used_pct": (
                round((total_used / context_window) * 100, 1) if context_window > 0 else 0
            ),
