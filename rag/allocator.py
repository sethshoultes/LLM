...
class TokenAllocator:
    ...
    def _allocate_equal(
        self, documents: List[Dict[str, Any]], available_tokens: int
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _allocate_proportional(
        ...
        for doc in documents:
            ...
            doc_allocation = max(DEFAULT_MIN_DOCUMENT_TOKENS, int(content_tokens * proportion))
    ...
    def _allocate_prioritized(
        ...
        for doc in sorted_docs:
            ...
            doc_tokens = doc.get(
                "total_tokens", token_manager.estimate_tokens(doc.get("content", ""))
            )
    ...
    def _allocate_adaptive(
        self, documents: List[Dict[str, Any]], available_tokens: int, query: str = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if len(sorted_docs) > 1 and "score" in sorted_docs[0]:
            ...
            top_doc_tokens = min(
                top_doc.get(
                    "total_tokens", token_manager.estimate_tokens(top_doc.get("content", ""))
                ),
            ...
            if remaining_tokens > header_tokens_per_doc + DEFAULT_MIN_DOCUMENT_TOKENS:
                ...
                top_allocated["truncated"] = top_doc_tokens < token_manager.estimate_tokens(
                    top_doc.get("content", "")
                )
