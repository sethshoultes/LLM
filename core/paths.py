    ...
    def _is_model_file(self, path: Path, format_filter: Optional[str] = None) -> bool:
        ...
        if format_filter:
            if format_filter.lower() == 'gguf' and path.suffix.lower() == '.gguf':
                return True
            elif format_filter.lower() == 'ggml' and path.suffix.lower() == '.bin' and 'ggml' in path.name.lower():
                return True
            elif format_filter.lower() == 'pytorch' and path.suffix.lower() in ['.safetensors', '.pt', '.bin'] and 'ggml' not in path.name.lower():
                return True
