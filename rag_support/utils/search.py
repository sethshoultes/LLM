...
class EnhancedSearch:
    ...
    def search(
        ...
        if project_manager and self.use_new_search:
            ...
            try:
                ...
                logger.error(
                    f"Error searching documents with new RAG components: {e}", exc_info=True
                )
    ...
    @timer
    def search_by_tags(
        self, project_id: str, tags: List[str], max_results: int = DEFAULT_MAX_RESULTS
    ) -> List[Dict[str, Any]]:
        """
        ...
        if project_manager and self.use_new_search:
            # Use new RAG components via project manager
            try:
                return project_manager.search_documents(
                    project_id, "", tags=tags, max_results=max_results
                )
            except Exception as e:
                logger.error(
                    f"Error searching documents by tags with new RAG components: {e}", exc_info=True
                )
                # Fall back to legacy implementation
    ...
    @timer
    def find_similar(
        self, project_id: str, doc_id: str, max_results: int = 5
    ) -> List[Dict[str, Any]]:
        """
    ...
    def extract_relevant_contexts(
        self, project_id: str, query: str, max_docs: int = 3, max_chars: int = 3000
    ) -> List[Dict[str, Any]]:
        """
            ...
            contexts.append(
                {
                    ...
                    "truncated": doc_token_count
                    < self.estimate_token_count(doc_title_text + content),
                }
            )
        ...
        for context in contexts:
            context["token_percentage"] = round(
                (context["tokens"] / total_tokens) * 100 if total_tokens > 0 else 0, 1
            )
    ...
    def _compute_document_scores(
        ...
        for token in query_tokens:
            token_docs = sum(
                1
                for doc in documents
                if token in self._tokenize(doc.get("content", "") + " " + doc.get("title", ""))
            )
            idf[token] = (
                math.log(doc_count / (token_docs + 1)) + 1.0
    ...
    def _load_documents(self, project_id: str) -> List[Dict[str, Any]]:
        ...
        if (
            cache_key in self.document_cache
